...
const session = createExpertSession({
  ...
  lab: {
    ...
    scenario: "원본 세 파일을 변경하지 않고 sanitized clone/fixture에서 dependency→conditions→beans→bootJar→runtime evidence를 만들며 secret remediation과 rollback runbook을 완성합니다.",
    setup: ["JDK 21", "원본과 호환되는 Gradle Wrapper", "격리된 작업 복사본", "synthetic configuration/비밀", "localhost disposable port", "원본 파일 read-only 권한"],
